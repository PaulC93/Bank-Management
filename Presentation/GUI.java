...
public class GUI {
    ...
    btnUpdateEmployee.addActionListener(new ActionListener() {
        ...
        menuGenerateReport.addMenuListener(new MenuListener() {
            @Override
            public void menuSelected(MenuEvent e) {
                setAllOptionsInvisible();
                setGenerateOptions(true);
            }
        ...
        btnGenerate.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                try{
                   ArrayList<Log> logList=LogManager.getLogs(Integer.parseInt(txtEmployeeID.getText()), txtFromDate.getText(), txtToDate.getText());
                    if (logList!=null)
                        if (!logList.isEmpty())
                        {
                            String[] columnNames={"ID","action","date"};
                            Object[][] data=new Object[logList.size()][3];
                            for(int i=0; i<logList.size();i++ )
                            {
                                Log log=logList.get(i);
                                data[i][0]=log.getEmployeeID();
                                data[i][1]=log.getAction();
                                data[i][2]=log.getDate();
                            }
                            addTable(columnNames,data);
                        }
